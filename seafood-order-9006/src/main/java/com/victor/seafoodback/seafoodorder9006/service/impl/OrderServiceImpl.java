...
public class OrderServiceImpl implements OrderService {
    ...
    public CommonResult addOrder1(Integer userId, Integer seafoodId, Integer addrId, String serialNumber) {
        ...
        orderDao.updateSeafood(seafoodId, 1);
        return new CommonResult(200, "生成订单成功");
    }

    @Override
    @Transactional
    public CommonResult addOrder2(Integer userId, String[] seafoodList, Integer addrId, Double pay, String serialNumber) {
        //TODO 还没有减少库存,销量增加
        Order order = new Order();
        order.setUserId(userId);
        order.setCreateDate(new Date());
        ...
        for (String s : seafoodList) {
            ...
            if (split.length == 1) {
                ...
                orderDao.updateSeafood(Integer.parseInt(split[0]), 1);
            } else {
                orderDao.addOrderSeafood(order.getId(), Integer.parseInt(split[0]), Integer.parseInt(split[1]));
                orderDao.updateSeafood(Integer.parseInt(split[0]), Integer.parseInt(split[1]));
            }
